...
public abstract class Options {
	...
	/**
	 * The available languages - the first language in the array is the default
	 * language.
	 */
	...
	public boolean languageIsFound = false;// use to detect a language in the initilalisation
	...
	public static final int MAX_RESULT_PAIRS = 1000;// max of store_matches
	...
	protected static void printAllLanguages() throws jplag.ExitException {
		for (int i = 0; i < languages.length - 1; i += 2)
			try {
				Language langClass = (Language) Class.forName(languages[i + 1]).newInstance();

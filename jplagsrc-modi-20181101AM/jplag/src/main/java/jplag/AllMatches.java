...
public class AllMatches extends Matches implements Comparator<AllMatches> {
    ...
    /*
     * Returns the name of the submissions which were compared Parameter: i == 0
     * submission A, i != 0 submission B.
     */
    ...
    /*
     * The bigger a match (length "anz") is relatively to the biggest match the
     * redder is the color returned by this method.
     */
    ...
    public final void HTMLreport(PrintWriter f, int matchnr, Program program) {
        ...
        f.println("<TR><TH><TH>" + subA.name + " (" + percentA() + "%)<TH>" + subB.name + " (" + percentB() + "%)<TH>"
        ...
        for (int i = 0; i < size(); i++) {
            ...
            f.print("<TD><A HREF=\"javascript:ZweiFrames('match" + matchnr + "-0.html#" + i + "',2,'match" + matchnr
                    + "-1.html#" + i + "',3)\" NAME=\"" + i + "\">");
            ...
            f.println(")</A><TD ALIGN=center>" + "<FONT COLOR=\"" + color(match.length) + "\">" + match.length
                    + "</FONT>");
        }
        if (program.useBasecode()) {
            f.print("<TR><TD BGCOLOR=\"#C0C0C0\"><TD>" + msg.getString("AllMatches.Basecode") + " "
                    + roundedPercentBasecodeA() + "%");
            f.println("<TD>" + msg.getString("AllMatches.Basecode") + " " + roundedPercentBasecodeB() + "%<TD>&nbsp;");
        }
        f.println("</TABLE>\n</CENTER>");
    }

    /*
     * This method generates an table which shows the distribution of match lengths.
     */
    ...
    public final void distributionReport(PrintWriter f, Messages msg) {
        ...
        f.println("<TR><TH ALIGN=center BGCOLOR=#8080ff>" + msg.getString("AllMatches.Length")
                + "<TH ALIGN=center BGCOLOR=#8080ff>" + msg.getString("AllMatches.Number_of_matches")
                + "<TH ALIGN=center BGCOLOR=#8080ff>.</TR>");
        for (int i = 0; i < maxLength; i++) {
            if (dist[i] == 0)
                continue;
            f.print("<TR><TD ALIGN=center BGCOLOR=#c0c0ff>" + (i + 1) + "<TD ALIGN=right BGCOLOR=#c0c0ff>" + dist[i]
                    + "<TD BGCOLOR=#c0c0ff>");
     ...
     * report. It returns: 0 : if the line "line" is not represented by a token. 1 :
     * if all the tokens from the line are covered by a single match. 2 : if at
     * least one token from the line does not belong to a match.
     */
    public final int diffType(String file, int line, int sub) {
        Structure struct = (sub == 0 ? subA : subB).struct;
        int index = 0;
        for (; index < struct.size(); index++)
            if (struct.tokens[index].file.equals(file) && struct.tokens[index].getLine() == line)
                break;
        ...
        while (index < struct.size() && struct.tokens[index].getLine() == line
            ...
            int j = 0;
            for (; j < size(); j++)
                if (matches[j].contains(index, sub))
                    break;
            if (j == size())
                return 2;
            index++;
    ...
    /*
     * This method returns the name of all files that are represented by at least
     * one token.
     */
